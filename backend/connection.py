...
class Connection:
    ...
    def get_unknown_stock(self, ticker, start = '2019-01-01',end = '2023-04-01'):
        ...
        df = df[['ticker','close','record_date']].sort_values(by='record_date')
        temp  = df.to_json(orient='records')
        parsed = json.loads(temp)
        return {'message':f"Data for {ticker} has been retrieved", 'data':parsed, 'raw_df':df}
        
        
    def insert_stock_data(self, data, engine):
        data.to_sql(name='stock_info_data', schema='stock_db', con =engine, if_exists='append', index=False, method = 'multi')
